import asyncio
import json
from quart import Blueprint,url_for, request, Response  # Quart has built-in SSE support
...
async def xlsx_upload_stream(job_id):
    ...
    if "text/event-stream" not in request.headers.get("Accept", ""):
        return "Expected Event-Stream", 406
    ...
    async def event_generator():
        # Check status from Redis (or your task tracker)

        if job_id not in services.tasks:
            yield f"""data: {json.dumps({"status": "job is not available with this task"})}\n\n"""
            return
        ...
        if services.tasks.get(job_id).get("status") == "failed":
            yield f"data: {json.dumps({'status': 'product upload failed'})}\n\n"
        else:
            ...
            if returned_value_of_submit_task.get("code") == 200:
                ...
                yield f"data: {json.dumps({
                                    ...
                                    "status": "some products could not be uploaded"
                                })}\n\n"
